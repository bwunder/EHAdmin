:on error exit
-------------------------------------------------------------------------------
-- Encryption Hierarchy Administration 
-------------------------------------------------------------------------------
-- Pre-requisites and useage (READ BEFORE RUNNING SCRIPT):
--  1. Existing ODBC System DSN to the offsite database is required.   
--  2. Replace template tokens - in Query menu "Specify Values For Template 
--     Parameters" 
--  3. TrueCrypt KEY_CONTAINER filemounted as VHD must be in place and available 
--     before executing script.(see http://www.truecrypt.org). The SQL Server 
--     service account needs full control of the Containers file and the VHD. 
--     Include password used as the TRUECRYPT_CONTAINER_ENCRYPTION_PHRASE var.
--     Estimating a generous 1.5KB/key and 3KB/certificate and given the Minimum 
--     Truecrypt NTFS volume is smallest size possible: 3792k, est > 2000 exports 
--  4. Scripts adds a Database Master Key and a certificate in the master db
--     on Enterprise Edition SQL Servers (used for TDE). For all Editions a linked 
--     server and linked server login are created, CLR is enable and 'max text 
--     repl size (B)' is set to -1 (size limited only by type)  
--  5. A user db is created or an existing db can be used. A pre-existing DMK 
--     will require the original passhrase as the master_DMK_ENCRYPTION_PHRASE var
--  6. Use this script in SSMS using SQLCMD mode only. (choose Query|SQLCMD Mode
--     in menu). To protect secrets. NEVER save the script once secrets entered. 
--  7. Passphrase hardness policy is located in the CheckPhrase function.
--  8. Algorithm defaults are best practice or otherwise called out as 
--     preferred in current industry and privacy standards, e.g. NIST: FIPS 140-2,
--     PCI-DSS, SAE16, see: 
--     http://blogs.msdn.com/b/ace_team/archive/2007/09/07/aes-vs-3des-block-ciphers.aspx
--  9. Enterprise, DataCenter, Developer or Evaluation Edition SQL Server 
--     required for TDE, Change Data Capture, SPARSE, FILESTREAM
-- 10. SQL Server 2012 required for FileTable, constant assignement in 
--     declarations, THROW, FORMATMESSAGE usage, SHA2_512 hashbytes algorithm
--     
-------------------------------------------------------------------------------
SET NOCOUNT ON;
-- private passphrase - is masked in parameter audit record, lost if you do not remeber it!
...
:setvar KEY_CONTAINER_PATH                     "C:\EHA\"                -- Full Control to SQL Server service account                  
:setvar KEY_CONTAINER_FILE                     "Container.eha"          -- truecrypt container created at or copied to above path      
:setvar TRUECRYPT_EXE                          "I:\TrueCrypt.exe"       -- thumb drive, optical media or share OK,no need to install   
:setvar VHD_LETTER                             "X"                                                                                     
...
:setvar AUDIT_SYMMETRIC_KEY                    "AuditKey"               -- "<[AUDIT_SYMMETRIC_KEY],SYSNAME,ParameterKey>"                                 
...
:setvar ERROR_KEY_ENCRYPTION_ALGORITHM         "AES_128"                -- "<[ERROR_KEY_ENCRYPTION_ALGORITHM],SYSNAME,AES_128>"                           
...
:setvar LINKED_SERVER                          "OffsiteLinkedServer"    -- "<[LINKED_SERVER],SYSNAME,OffsiteLinkedServer>"                                
...
:setvar WITH_OPTIONS                           "WITH EXECUTE AS CALLER"--, ENCRYPTION" -- FOR LIVE DATA ALWAYS ENCRYPT STORED PROCEDURES
GO  
-- the ODBC DSN must be created using ODBCad32.exe (should be in your %PATH% already)
...
GO
-- if truecyrpt cannot dismount any existing vhd the script will stop
...
IF NOT EXISTS( SELECT * 
  ...
  RAISERROR('Only a Windows authenticated member of the sysadmin fixed server role should execute this script.',16, 1);
GO      
IF ISNULL(PARSENAME ( CONVERT(NVARCHAR(128), SERVERPROPERTY('ProductVersion')) , 4 ), 0) < 11
  RAISERROR('SQL Server 2012 is required. See this scripts header notes for details.',16, 1);
GO
-- self-signed certificates = easier prey for authentication relay exploits (aka man in the middle)
IF (SELECT UPPER(encrypt_option) FROM sys.dm_exec_connections WHERE session_id = @@spid) = 'FALSE'
  RAISERROR('Unencrypted connections increase the risk of parameter and returned data exposure on the network.  see http://msdn.microsoft.com/en-us/library/ms191192.aspx' ,0 ,0);
GO
IF NOT EXISTS ( SELECT * FROM sys.configurations 
                WHERE name = 'filestream access level'
                AND value = 2)
  RAISERROR('FILESTREAM for Transact-SQL and Win32 streaming access must be enabled in SQL Server Configuration Manager.',16, 1);
GO
RAISERROR('Confirm that no device or software has captured your PASSPHRASEs  see http://wskills.blogspot.com/2007/01/how-to-find-fight-keyloggers.html and http://msdn.microsoft.com/en-us/library/ff648641.aspx',0,0);
GO
...
IF PATINDEX('%[Developer,Enterprise]%', CAST(SERVERPROPERTY('Edition') AS NVARCHAR(128) ) ) > 0 
  BEGIN
    -- Certificate in master for TDE encrypted by master DMK (which is encrypted by the SMK(which is encrypted by the DPAPI)) 
...
-- cleared when the db closes (when last user disconnectes). AUTO_CLOSE will also provide a backup 
...
GO
-- intended for use only by mad scientists at M$ labs and the catatonically insane 
...
IF (SELECT owner_sid FROM sys.databases where name = DB_NAME()) <> 0x01
  BEGIN
    ...
    -- could already be another trusted 3 party authenticator in use
    IF ( SELECT UPPER(auth_scheme)  FROM sys.dm_exec_connections 
         WHERE session_id = @@spid ) <> 'KERBEROS'
      RAISERROR('Consider using the "KERBEROS" authentication scheme.  see http://blogs.msdn.com/b/sql_protocols/archive/2006/12/02/understanding-kerberos-and-ntlm-authentication-in-sql-server-connections.aspx',0,0);
    ...
    IF ( SELECT value FROM sys.configurations
      ...
      RAISERROR('Consider enabling Common Criteria compliance.  see http://msdn.microsoft.com/en-us/library/bb326650(v=SQL.110).aspx', 0, 0);
...
IF IS_MEMBER('$(EHADMIN_ROLE)') <> 1
  BEGIN
    ...
    --2008--SET @AddToRoleDDL = 'EXEC sp_addrolemember ''$(EHADMIN_ROLE)'',[' + ORIGINAL_LOGIN() + ']';
...
-- see sp_control_dbmasterkey_password documention for some security comments
GO
...
IF NOT EXISTS (SELECT * FROM sys.certificates WHERE name = '$(AUDIT_CERTIFICATE)')
  ...
  WITH SUBJECT = 'Encryption Hieararchy Administrator Audit Trail';
GO
-- audit certificate is independent FROM DMK 
-- even so is sticky to instance without first exporting public & private key 
IF NOT EXISTS (SELECT * 
               FROM sys.symmetric_keys 
...
IF OBJECT_ID('$(EHA_SCHEMA).$(BOOKINGS_TABLE)') IS NULL
  BEGIN
	   CREATE TABLE $(EHA_SCHEMA).$(BOOKINGS_TABLE) 
        ...
        , ErrorData VARBINARY(8000) SPARSE NULL
		    ...
		    , CONSTRAINT pkn_$(BOOKINGS_TABLE)__Id__ServerName
		      PRIMARY KEY NONCLUSTERED (Id, ServerName) );
  ...
  END
GO
IF NOT EXISTS (SELECT * FROM sys.indexes where name = 'ixc_$(BOOKINGS_TABLE)__CreateUTCDT__ServerName')
  CREATE CLUSTERED INDEX ixc_$(BOOKINGS_TABLE)__CreateUTCDT__ServerName
...
EXEC sp_addmessage $(MESSAGE_OFFSET)01, 1, '@tvp.Name = ''%s'', @tvp.Value = ''%s'' @Version = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).AddNameValue, $(EHA_SCHEMA).ValidateNameValue
EXEC sp_addmessage $(MESSAGE_OFFSET)02, 1, '@NameBucket = %d, @Version = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).SelectNameValue
EXEC sp_addmessage $(MESSAGE_OFFSET)03, 1, '@MAC = %s', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).CertificateBackupsByThumbprint
EXEC sp_addmessage $(MESSAGE_OFFSET)04, 1, '@Name = ''%s'', @Value = ''%s'', @EncryptionPhrase = ''%s''', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).SavePrivateValue, $(EHA_SCHEMA).GetPrivateValue
EXEC sp_addmessage $(MESSAGE_OFFSET)05, 1, '@DBName = ''%s'', @SchemaName= ''%s'', @TableName = ''%s'', @ColumnName = ''%s''', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).MakeSalt
EXEC sp_addmessage $(MESSAGE_OFFSET)06, 1, '@KeyName = ''%s'', @KeyIdentity = ''%s'', @KeySource = ''%s''', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).SavePortableKey
EXEC sp_addmessage $(MESSAGE_OFFSET)07, 1, '@FileName = ''%s'', @Tag = ''%s''', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).BackupContainer
EXEC sp_addmessage $(MESSAGE_OFFSET)08, 1, '@ServerName = ''%s''', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).BackupContainer, Reports
EXEC sp_addmessage $(MESSAGE_OFFSET)09, 1, '@Id = ''%s'', @FileName = ''%s''', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).RecallContainer
...
--encryption hierarchy @Parameters
EXEC sp_addmessage $(MESSAGE_OFFSET)11, 1, '@BackupPhrase = ''%s'', @UseHash = %d, @ForceNew = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).BackupServiceMasterKey
EXEC sp_addmessage $(MESSAGE_OFFSET)12, 1, '@IdToRestore = %d, @ForceRestore = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).RestoreServiceMasterKey
EXEC sp_addmessage $(MESSAGE_OFFSET)13, 1, '@DbName = ''%s'', @BackupPhrase = ''%s'', @KeyPhrase = ''%s'', @UseHash = %d, @ForceNew = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).BackupDatabaseMasterKey
EXEC sp_addmessage $(MESSAGE_OFFSET)14, 1, '@DbName = ''%s'', @IdToRestore = %d, @ForceReplace = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).RestoreDatabaseMasterKey
EXEC sp_addmessage $(MESSAGE_OFFSET)15, 1, '@CertificateName = ''%s'', @DbName ''%s'', @BackupPhrase ''%s'', @KeyPhrase ''%s'' , @UseHash = %d, @ForceNew = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).BackupCertificate
EXEC sp_addmessage $(MESSAGE_OFFSET)16, 1, '@CertificateName = ''%s'', @DbName ''%s'', @IdToRestore = %d', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).RestoreCertificate
--salt name
EXEC sp_addmessage $(MESSAGE_OFFSET)17, 1, '%s.%s.%s.%s.Salt', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).AddSalt
-- for FORMATMESSAGE of ErrorData
EXEC sp_addmessage $(MESSAGE_OFFSET)18, 1, 'Error: %d, %d, %d Obj: %s, line: %d, msg: %s', 'us_english','FALSE' ,'replace' -- $(EHA_SCHEMA).AddSalt
-- encryption hierarchy backups
EXEC sp_addmessage $(MESSAGE_OFFSET)21, 1, '%s %s %s %s %s complete.', 'us_english','FALSE' ,'replace'
EXEC sp_addmessage $(MESSAGE_OFFSET)22, 16, '%s %s %s %s %s failed with return_code %d.', 'us_english','FALSE' ,'replace'
EXEC sp_addmessage $(MESSAGE_OFFSET)23, 16, '%s %s %s %s %s failed with reason %s.', 'us_english','FALSE' ,'replace'
EXEC sp_addmessage $(MESSAGE_OFFSET)24, 16, 'Request for duplicate encryption hierarchy node Backup must use @ForceNew = 1 (%s %s)', 'us_english','FALSE' ,'replace'
...
EXEC sp_addmessage $(MESSAGE_OFFSET)37, 16, 'ANSI_PADDING is OFF. Not suited for SQL Server encryption. See BOL', 'us_english','FALSE' ,'replace'
-- more encryption hierarchy backups
EXEC sp_addmessage $(MESSAGE_OFFSET)43, 1, 'OPEN MASTER KEY DECRYPTION BY PASSWORD = ''%s'';', 'us_english','FALSE' ,'replace'
EXEC sp_addmessage $(MESSAGE_OFFSET)44, 1, '$(EHA_SCHEMA).AddSalt( ''%s'', ''%s'', ''%s'', ''%s'', %s )', 'us_english','FALSE' ,'replace'
EXEC sp_addmessage $(MESSAGE_OFFSET)45, 1, 'SELECT @CipherType = key_algorithm, @Colophon = %s FROM master.sys.symmetric_keys WHERE name = ''%s'';', 'us_english','FALSE' ,'replace'
EXEC sp_addmessage $(MESSAGE_OFFSET)46, 1, 'SELECT @CipherType = pvt_key_encryption_type, @Colophon = %s FROM %s.sys.certificates WHERE name = ''%s'';', 'us_english','FALSE' ,'replace'
GO
-- sign messages
DECLARE @InstallValues NVARCHAR(4000),@Encrypted VARBINARY(8000);
SET @InstallValues = ':setvar KEY_CONTAINER_FILE "$(KEY_CONTAINER_FILE)"' + CHAR(13) + CHAR(10)
      + ':setvar KEY_CONTAINER_PATH "$(KEY_CONTAINER_PATH)"' + CHAR(13) + CHAR(10)
      + ':setvar TRUECRYPT_EXE "$(TRUECRYPT_EXE)"' + CHAR(13) + CHAR(10)
      + ':setvar VHD_LETTER "$(VHD_LETTER)"' + CHAR(13) + CHAR(10)
      -- ODBC DSN  to offsite backup' + CHAR(13) + CHAR(10)
      + ':setvar LINKED_SERVER_ODBC_DSN "$(LINKED_SERVER_ODBC_DSN)"' + CHAR(13) + CHAR(10)
      + ':setvar LINKED_SERVER "$(LINKED_SERVER)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_USER "$(LINK_USER)"' + CHAR(13) + CHAR(10)
      -- master database encryption hierarchy (certificate is needed for TDE)' + CHAR(13) + CHAR(10)
      + ':setvar TDE_CERTIFICATE "$(TDE_CERTIFICATE)"' + CHAR(13) + CHAR(10)
      + ':setvar TDE_CERTIFICATE_ALGORITHM "$(TDE_CERTIFICATE_ALGORITHM)"' + CHAR(13) + CHAR(10)
      -- EHDB database encryption hierarchy' + CHAR(13) + CHAR(10)
      + ':setvar AUDIT_CERTIFICATE "$(AUDIT_CERTIFICATE)"' + CHAR(13) + CHAR(10)
      + ':setvar AUDIT_SYMMETRIC_KEY "$(AUDIT_SYMMETRIC_KEY)"' + CHAR(13) + CHAR(10)
      + ':setvar AUDIT_KEY_ENCRYPTION_ALGORITHM "$(AUDIT_KEY_ENCRYPTION_ALGORITHM)"' + CHAR(13) + CHAR(10)
      + ':setvar AUTHENTICITY_CERTIFICATE "$(AUTHENTICITY_CERTIFICATE)"' + CHAR(13) + CHAR(10)
      + ':setvar ERROR_SYMMETRIC_KEY "$(ERROR_SYMMETRIC_KEY)"' + CHAR(13) + CHAR(10)
      + ':setvar ERROR_KEY_ENCRYPTION_ALGORITHM "$(ERROR_KEY_ENCRYPTION_ALGORITHM)"' + CHAR(13) + CHAR(10)
      + ':setvar ERROR_KEY_SOURCE "$(ERROR_KEY_SOURCE)"' + CHAR(13) + CHAR(10)
      + ':setvar ERROR_KEY_IDENTITY "$(ERROR_KEY_IDENTITY)"' + CHAR(13) + CHAR(10)
      + ':setvar FILE_CERTIFICATE "$(FILE_CERTIFICATE)"' + CHAR(13) + CHAR(10)
      + ':setvar FILE_SYMMETRIC_KEY "$(FILE_SYMMETRIC_KEY)"' + CHAR(13) + CHAR(10)
      + ':setvar FILE_KEY_ENCRYPTION_ALGORITHM "$(FILE_KEY_ENCRYPTION_ALGORITHM)"' + CHAR(13) + CHAR(10)
      + ':setvar HASHBYTES_ALGORITHM "$(HASHBYTES_ALGORITHM)"' + CHAR(13) + CHAR(10)
      + ':setvar MIN_PHRASE_LENGTH "$(MIN_PHRASE_LENGTH)"' + CHAR(13) + CHAR(10)
      + ':setvar NAME_CERTIFICATE "$(NAME_CERTIFICATE)"' + CHAR(13) + CHAR(10)
      + ':setvar NAME_SYMMETRIC_KEY "$(NAME_SYMMETRIC_KEY)"' + CHAR(13) + CHAR(10)
      + ':setvar NAME_KEY_ENCRYPTION_ALGORITHM "$(NAME_KEY_ENCRYPTION_ALGORITHM)"' + CHAR(13) + CHAR(10)
      + ':setvar OBJECT_CERTIFICATE "$(OBJECT_CERTIFICATE)"'+ CHAR(13) + CHAR(10)
      + ':setvar USE_HASH_FOR_FILENAME "$(USE_HASH_FOR_FILENAME)"' + CHAR(13) + CHAR(10)
      + ':setvar VALUE_CERTIFICATE "$(VALUE_CERTIFICATE)"' + CHAR(13) + CHAR(10)
      + ':setvar VALUE_SYMMETRIC_KEY "$(VALUE_SYMMETRIC_KEY)"' + CHAR(13) + CHAR(10)
      + ':setvar VALUE_KEY_ENCRYPTION_ALGORITHM "$(VALUE_KEY_ENCRYPTION_ALGORITHM)"' + CHAR(13) + CHAR(10)
      -- DATABASE SCHEMA' + CHAR(13) + CHAR(10)
      + ':setvar EHDB "$(EHDB)"' + CHAR(13) + CHAR(10)
      + ':setvar EHA_SCHEMA "$(EHA_SCHEMA)"' + CHAR(13) + CHAR(10)
      + ':setvar EHADMIN_ROLE "$(EHADMIN_ROLE)"' + CHAR(13) + CHAR(10)
      + ':setvar BOOKINGS_TABLE "$(BOOKINGS_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar BACKUP_ACTIVITY_TABLE "$(BACKUP_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar CONTAINERS_TABLE "$(CONTAINERS_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar CONTAINER_ACTIVITY_TABLE "$(CONTAINER_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar FILESTREAM_FILEGROUP "$(FILESTREAM_FILEGROUP)"' + CHAR(13) + CHAR(10)
      + ':setvar FILESTREAM_FILE "$(FILESTREAM_FILE)"' + CHAR(13) + CHAR(10)
      + ':setvar FILETABLE_BACKUPS "$(FILETABLE_BACKUPS)"' + CHAR(13) + CHAR(10)
      + ':setvar FILETABLE_DIRECTORY "$(FILETABLE_DIRECTORY)"' + CHAR(13) + CHAR(10)
      --+ ':setvar FILETABLE_MAPPED_DRIVE_LETTER "$(FILETABLE_MAPPED_DRIVE_LETTER)"' + CHAR(13) + CHAR(10)
      + ':setvar RESTORE_FILETABLE "$(RESTORE_FILETABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar NAMEVALUES_TABLE "$(NAMEVALUES_TABLE)"' + CHAR(13) + CHAR(10)   
      + ':setvar NAMEVALUE_ACTIVITY_TABLE "$(NAMEVALUE_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar OFFSITE_ACTIVITY_TABLE "$(OFFSITE_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar REPORT_ACTIVITY_TABLE "$(REPORT_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar WITH_OPTIONS "WITH_OPTIONS"' + CHAR(13) + CHAR(10)
      + '-- Other CONFIGURATION' + CHAR(13) + CHAR(10)
      + ':setvar EVENT_NOTIFICATION "$(EVENT_NOTIFICATION)"' + CHAR(13) + CHAR(10)
      + ':setvar MESSAGE_OFFSET "$(MESSAGE_OFFSET)"' + CHAR(13) + CHAR(10)
      + ':setvar MASTER_KEY_BACKUP_EXT "$(MASTER_KEY_BACKUP_EXT)"' + CHAR(13) + CHAR(10)
      + ':setvar PUBLIC_KEY_BACKUP_EXT "$(PUBLIC_KEY_BACKUP_EXT)"' + CHAR(13) + CHAR(10)
      + ':setvar PRIVATE_KEY_BACKUP_EXT "$(PRIVATE_KEY_BACKUP_EXT)"' + CHAR(13) + CHAR(10)
      + ':setvar WITH_OPTIONS "$(WITH_OPTIONS)"' + CHAR(13) + CHAR(10)
      -- OFFSITE SCHEMA' + CHAR(13) + CHAR(10)
      + ':setvar LINK_EHDB "$(LINK_EHDB)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_SCHEMA "$(LINK_SCHEMA)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_BOOKINGS_SYNONYM "$(LINK_BOOKINGS_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_BOOKINGS_TABLE "$(LINK_BOOKINGS_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_BACKUP_ACTIVITY_SYNONYM "$(LINK_BACKUP_ACTIVITY_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_BACKUP_ACTIVITY_TABLE "$(LINK_BACKUP_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_CONTAINERS_SYNONYM "$(LINK_CONTAINERS_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_CONTAINERS_TABLE "$(LINK_CONTAINERS_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_CONTAINER_ACTIVITY_SYNONYM "$(LINK_CONTAINER_ACTIVITY_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_CONTAINER_ACTIVITY_TABLE "$(LINK_CONTAINER_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_NAMEVALUES_SYNONYM "$(LINK_NAMEVALUES_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_NAMEVALUES_TABLE "$(LINK_NAMEVALUES_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_NAMEVALUE_ACTIVITY_SYNONYM "$(LINK_NAMEVALUE_ACTIVITY_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_NAMEVALUE_ACTIVITY_TABLE "$(LINK_NAMEVALUE_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_OFFSITE_ACTIVITY_SYNONYM "$(LINK_OFFSITE_ACTIVITY_SYNONYM)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_OFFSITE_ACTIVITY_TABLE "$(LINK_OFFSITE_ACTIVITY_TABLE)"' + CHAR(13) + CHAR(10)
      + ':setvar LINK_REPORT_ACTIVITY_SYNONYM "$(LINK_REPORT_ACTIVITY_SYNONYM)"'
...
GO
-------------------------------------------------------------------
-- proc gets special slot in install because so many dependencies
-------------------------------------------------------------------
...
--    add filtering as appropriate. Be precise.  
-------------------------------------------------------------------------------
CREATE PROCEDURE $(EHA_SCHEMA).Book 
  ( @ProcId INT 
  , @Parameters VARBINARY(8000)
  , @Id NCHAR(36)
  , @Signedreferences BIT
...
DECLARE @Thumbprint VARBINARY(32)
  ...
  BEGIN TRY
    ...
    -- with minimizal added code, very low cost to query sys.objects by id
    IF (SELECT @SignedReferences                                        
    ...
    OR EXISTS ( SELECT * 
                ...
                WHERE name <> '$(EVENT_NOTIFICATION)Srv' ) 
    OR EXISTS ( SELECT * 
                FROM sys.triggers 
    ...
    OR (SELECT 1                                        -- maintain in lock step with schema changes
        ...
        WHERE OBJECT_SCHEMA_NAME( t.object_id ) = 'eha' -- by not using the function here the validity
        AND (parent_object_id = 0 OR type = 'TR')       -- of the function is assured
        HAVING COUNT(*) = 55
        AND SUM (IS_OBJECTSIGNED( 'OBJECT', t.object_id, 'CERTIFICATE', @Thumbprint ) )  = 47
        AND SUM ( CASE WHEN type = 'SN' THEN 1 ELSE 0 END ) = 8
        AND SUM ( CASE WHEN type = 'TR' THEN 1 ELSE 0 END ) = 8 ) <> 1           
    ...
    SET @Reason = 'sign';
    SET @MAC = (SELECT SignByCert( CERT_ID('$(AUTHENTICITY_CERTIFICATE)')
                                 , CAST(CHECKSUM( Id
                                                , ProcId   
                                                , ObjectName
                                                , Parameters
  ...
  BEGIN CATCH
    ...
    INSERT INTO $(EHA_SCHEMA).$(BOOKINGS_TABLE) 
      ...
      , Status
      , ErrorData )
    SELECT CASE WHEN EXISTS(SELECT * FROM $(EHA_SCHEMA).$(BOOKINGS_TABLE) WHERE Id = @Id)
                THEN CAST( NEWID() AS NCHAR(36) )
                ELSE @Id
                END
         , @ProcId
         , OBJECT_NAME(@ProcId)
         ...
         , ENCRYPTBYKEY( KEY_GUID('$(ERROR_SYMMETRIC_KEY)')
                       ...
                       , 1
                       , @Id )
    FROM (SELECT FORMATMESSAGE( $(MESSAGE_OFFSET)18
                              , ERROR_NUMBER()
                              , ERROR_SEVERITY()
                              , ERROR_STATE()
    ...
    IF @@NESTLEVEL > 1
      RAISERROR ($(MESSAGE_OFFSET)33, 16, 1, @Reason);
  END CATCH
END
GO
...
DECLARE @ConversationHandle UNIQUEIDENTIFIER
      ...
      , @Parameters VARBINARY(8000)
      , @ReturnCode INT
      , @SignedReferences BIT = $(EHA_SCHEMA).VerifySchema(@@PROCID)
      , @ServiceName NVARCHAR(128)
      , @tvp NAMEVALUETYPE
      , @Version INT; -- only for the unattended AddNameValue output parameter
SET NOCOUNT ON;
  BEGIN TRY
    OPEN SYMMETRIC KEY [$(AUDIT_SYMMETRIC_KEY)]
    DECRYPTION BY CERTIFICATE [$(AUDIT_CERTIFICATE)]
            ...
            BEGIN  
              ...
              AND @ServiceName = '$(EVENT_NOTIFICATION)Service'
                BEGIN
                  ...
                  IF (@MessageBody.value('(/EVENT_INSTANCE/EventType)[1]','NVARCHAR(128)') = 'AUDIT_DATABASE_OBJECT_MANAGEMENT_EVENT')           
  ...
  BEGIN CATCH
    ...
    SELECT @Id
         ...
         , ISNULL( @MessageBody, '<empty />' )
         , ISNULL( SIGNBYCERT( KEY_ID( '$(EVENT_CERTIFICATE)' )
                             , @MessageBody.value ( '(/EVENT_INSTANCE/TSQLCommand/CommandText)[1]','nvarchar(max)' ) )
                  , 0x0 )     
         , ISNULL( @MAC, 0x0 )
         , OBJECT_NAME( @@PROCID )
         , 'Complete'
  ...
  BEGIN CATCH
    ...
    SELECT ISNULL(d.Id, NEWID())
         ...
         , ENCRYPTBYKEY( KEY_GUID('$(ERROR_SYMMETRIC_KEY)')
                       ...
                       , @Id )
    FROM deleted d
    OUTER APPLY (SELECT FORMATMESSAGE( $(MESSAGE_OFFSET)18
                                     , ERROR_NUMBER()
                                     , ERROR_SEVERITY()
                                     , ERROR_STATE()
                                     , ISNULL(ERROR_PROCEDURE(), 'ad hoc')
...
AS
BEGIN
  DECLARE @SaltName NVARCHAR(443) = FORMATMESSAGE( $(MESSAGE_OFFSET)17
                                                 , @DbName
                                                 , @SchemaName
                                                 , @TableName
                                                 , @ColumnName);
  RETURN (SELECT ABS( CHECKSUM( HASHBYTES( '$(HASHBYTES_ALGORITHM)'
          ...
          FROM ( SELECT Id, Name, Value                                                                  
                 ...
                 WHERE NameBucket = ABS( CHECKSUM( HASHBYTES( '$(HASHBYTES_ALGORITHM)'
                                                            , RIGHT( @SaltName 
                                                                   , FLOOR( LEN( @SaltName ) / 2 ) )))) ) AS derived );
END
...
--    ASSERT: terminating reverse solidus "\" is always required
-------------------------------------------------------------------------------
...
AS
BEGIN 
  -- returns null if filetable empty
  DECLARE @Path NVARCHAR(1024);
  SET @Path = ( SELECT '$(VHD_LETTER):\'  
                FROM sys.certificates c
  ...
  RETURN (ENCRYPTBYKEY( KEY_GUID( '$(FILE_SYMMETRIC_KEY)' ), @Path, 1, @DbName ) );
END
GO
...
AS
BEGIN
  RETURN (SELECT IS_OBJECTSIGNED('OBJECT', @@PROCID, 'CERTIFICATE', c.thumbprint)
          FROM sys.certificates c
          JOIN sys.crypt_properties cp
          ON c.thumbprint = cp.thumbprint
          WHERE c.name = '$(OBJECT_CERTIFICATE)'
...
BEGIN
  DECLARE @Status NVARCHAR(30)
        ...
        , @Value NVARCHAR(128) ;
  -- dft password policy as described in 2008R2 BOL + SQL Injection prevention
  -- fyi: SELECT CAST(newid() AS VARCHAR(128)) returns a valid but weakened password 
  SET @Status = 'role';
  IF EXISTS (SELECT *              
              FROM sys.database_role_members
              WHERE role_principal_id = USER_ID('$(EHADMIN_ROLE)')
    ...
    BEGIN
      ...
      IF (SELECT IS_OBJECTSIGNED('OBJECT', @@PROCID, 'CERTIFICATE', c.thumbprint)
          FROM sys.certificates c
          JOIN sys.crypt_properties cp
          ON c.thumbprint = cp.thumbprint
          WHERE c.name = '$(OBJECT_CERTIFICATE)'
      ...
      BEGIN
        ...
        OR ( PATINDEX('%.SALT'    , UPPER(@Name) )           -- salt will fall through this check 
           + PATINDEX('%.IDENTITY', UPPER(@Name) )           -- portable key identity will fall through  
           + PATINDEX('%.SOURCE'  , UPPER(@Name) )           -- portable key source will fall through
           + PATINDEX('%.PRIVATE' , UPPER(@Name) ) > 0 )     -- ASavePrivateValue 
          BEGIN 
            -- black list is never strong but for sysadmin ad hoc restriction a little T-SQL black list 
            SET @Status = 'injection';                       
            IF ( PATINDEX('%[__"'']%', @UpValue)             -- no underscore (so no sp_ or xp_)  or quotes
               + PATINDEX('%DROP%'   , @UpValue)             -- none of these multi-character commands 
               ...
               + PATINDEX('%--%'     , @Value)                -- no comments
               + PATINDEX('%**/%'    , @Value) 
               + PATINDEX('%/**%'    , @Value)  = 0 )
            OR ( PATINDEX('%.SALT'    , UPPER(@Name) )           -- salt will fall through this check 
               + PATINDEX('%.IDENTITY', UPPER(@Name) )           -- portable key identity will fall through  
               + PATINDEX('%.SOURCE'  , UPPER(@Name) )           -- portable key source will fall through
               + PATINDEX('%.PRIVATE' , UPPER(@Name) ) > 0 )     -- SavePrivateValue                 
               BEGIN 
...
BEGIN
  DECLARE @NewName NVARCHAR(448) =  
                 ( SELECT REPLACE(@@SERVERNAME,'\','$') 
	                      + '__' +
	                      + CASE WHEN @DbName = 'master' 
                               AND @NodeName = 'Service Master Key'
                               AND @Purpose = 'Encryption' THEN NULL -- invalid combination
                               WHEN @DbName = 'master' 
                               AND @NodeName = 'Service Master Key' 
                               THEN 'SERVICE_MASTER_KEY' 
		                           WHEN @DbName IS NOT NULL 
                               AND @NodeName = 'Database Master Key'
                               THEN @DbName + '__DATABASE_MASTER_KEY'
				                       WHEN @DbName IS NOT NULL 
                               AND @NodeName IS NOT NULL 
                               THEN @DbName + '__' + @NodeName END 
   		                  + '__'
...
-- SMK automatically generated during SQL Server Setup
-- DMK
--  (| - encrypted by parent ) 
--  (X - no cryptographic dependency)        Node
-------------------------------------------------------------------------------
--root                                                      \
--   |_     Service Master Key A .......................... \1                                              
--   | |_   Database I Master Key ......................... \1\1                     
--   | X |_ Certificate a                                   \1\1\1                    
--   | X X_ Certificate b                                   \1\1\2                              
--   | X X_ Certificate c                                   \1\1\3
--   | X |_ Certificate d                                   \1\1\4
--   | X_   Database B Master Key ......................... \1\2                    
--   |   X_ Certificate b                                   \1\2\1
--   |   |_ Certificate e                                   \1\2\2                             
--   |
--   |_     Service Master Key B .......................... \2
--   | |_   Database II Master Key ........................ \2\1
--   | | |_ Certificate a                                   \2\1\1
--   | |_   Database III Master Key ....................... \2\2
--   |   |_ Certificate b                                   \2\2\1
--   | 
--   |_     Service Master Key C .......................... \2
--   | X_   .................... no DMK ................... \2\1
--   |   X_ Certificate a                                   \2\1\1
--   |
--   |__ Service Master Key .............................. \n
--      ?__Database Master Key ........................... \n\n
--         ?__Certificate ................................ \n\n\n
-------------------------------------------------------------------------------
...
BEGIN
  ...
  WHILE @Retry < 3
    BEGIN
      SET @NewName = (SELECT REPLACE(@@SERVERNAME,'\','$') + '__' 
                             + @DbName + '__' 
                             + @NodeName + '__' 
                             + REPLACE( REPLACE( REPLACE( CONVERT( NVARCHAR(30)
                                                                 , SYSUTCDATETIME()
                                                                 , 21)
                                                        , SPACE(1), '__')
                                               , '.','-')          
                      ...
                      AND EXISTS (SELECT *              
                                  ...
                                  AND SYSTEM_USER = ORIGINAL_LOGIN()
                                  AND USER_NAME( member_principal_id ) = ORIGINAL_LOGIN() ) );       
      IF ( $(EHA_SCHEMA).CheckFile(@NewName, @DbName) = 1 )
        BREAK;
      SET @NewName = NULL;
      SET @Retry += 1;
    END  
...
BEGIN
  ...
  WHILE @Retry < 3
    BEGIN
  SET @NewName = (SELECT REPLACE(@@SERVERNAME,'\','$') + '__' 
                       + CASE WHEN @DbName is NULL 
                             THEN 'ServiceMasterKey'
                             ELSE @DbName + '__MasterKey' END + '__' 
                       + REPLACE( REPLACE( REPLACE( 
                         CONVERT( NVARCHAR(30), SYSUTCDATETIME(), 21)
                         , SPACE(1), '__'), '.','-'), ':', '-') 
                  FROM sys.certificates c
                  JOIN sys.crypt_properties cp
                  ON c.thumbprint = cp.thumbprint
      ...
      SET @Retry += 1;
    END  
   RETURN (ENCRYPTBYKEY( KEY_GUID('$(FILE_SYMMETRIC_KEY)'), @NewName ) );
END
GO
  ...
  BEGIN TRY
    ...
    SET @Parameters = 
      ( SELECT ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                           , FORMATMESSAGE( $(MESSAGE_OFFSET)01
...
DECLARE @ErrorData VARBINARY(8000)
      ...
      , @Parameters VARBINARY(8000)
      , @ReturnCode INT
      , @SaltName NVARCHAR(448) = FORMATMESSAGE( $(MESSAGE_OFFSET)17
                                               , @DbName
                                               , @SchemaName
                                               , @TableName
                                               , @ColumnName )
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)' )
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)05
    ...
    IF @ReturnCode <> 0
      RAISERROR( $(MESSAGE_OFFSET)22, 16, 1
...
CREATE PROCEDURE $(EHA_SCHEMA).SavePortableSymmetricKey 
 ...
 , @KeyIdentity NVARCHAR(128)
 , @KeySource NVARCHAR(128) )
$(WITH_OPTIONS)
AS
BEGIN 
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)' )
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)06
                                                 , @KeyName
                                                 , @KeyIdentity
                                                 , @KeySource )
                                  , 1
    ...
    VALUES( ENCRYPTBYKEY( KEY_GUID('$(NAME_SYMMETRIC_KEY)')
          ...
          , ENCRYPTBYKEY( KEY_GUID('$(VALUE_SYMMETRIC_KEY)')
                        , @KeyIdentity, 1, FORMATMESSAGE( '%s.Identity'
                                                        , @KeyName ) ) );
    ...
    VALUES( ENCRYPTBYKEY( KEY_GUID('$(NAME_SYMMETRIC_KEY)')
          ...
          , ENCRYPTBYKEY( KEY_GUID('$(VALUE_SYMMETRIC_KEY)')
                        , @KeySource, 1, FORMATMESSAGE( '%s.Source'
                                                      , @KeyName ) ) );
    EXEC $(EHA_SCHEMA).AddNameValue @tvp, @Version OUTPUT;   
    INSERT $(EHA_SCHEMA).$(NAMEVALUE_ACTIVITY_TABLE) 
      ( Id
...
CREATE PROCEDURE $(EHA_SCHEMA).SavePrivateValue 
 ( @Name NVARCHAR(448) 
 , @Value NVARCHAR(128) 
 , @EncryptionPhrase NVARCHAR(128) 
 , @AuditPrivateData BIT = 0 )  
$(WITH_OPTIONS)
AS
BEGIN 
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)' )
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)04
                                                 , CASE 
                                                   WHEN @AuditPrivateData = 1 
                                                   THEN  @Name
                                                   ELSE N'DATAVALUE_NOT_AUDITED'
                                                   END ) 
                                  , 1
                                  , @Id );
    CLOSE SYMMETRIC KEY [$(AUDIT_SYMMETRIC_KEY)];
    ...
    SET @Value = CAST( ENCRYPTBYPASSPHRASE( @EncryptionPhrase, @Value) AS NVARCHAR(128) ) 
    INSERT @tvp 
      ( Name
      , Value) 
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    SELECT Name 
         , CAST( DECRYPTBYPASSPHRASE( @EncryptionPhrase
                                    , LEFT( Value
...
--    Desc: backup the service master key of the SQL Server Instance
--      specify    @UseHash = 1
--      use name that identifies backup or checksum of it's hash as file name
--      force a backup even if current version of key is already backed up 
-------------------------------------------------------------------------------
CREATE PROCEDURE $(EHA_SCHEMA).BackupServiceMasterKey 
 ( @BackupPhrase NVARCHAR(128) 
 , @UseHash TINYINT = 0    -- readable Name (0) or Bucket # (1) as the file name
 , @ForceNew TINYINT = 0 ) -- backup even if thw crypto object already on file
$(WITH_OPTIONS)
AS
BEGIN 
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)' )
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)11
                                                 , @BackupPhrase
                                                 , @UseHash
                                                 , @ForceNew )
                                  , 1
    ...
    SET @ObjectInfoDDL = FORMATMESSAGE( $(MESSAGE_OFFSET)45
                                      , FORMATMESSAGE( $(MESSAGE_OFFSET)44
                                                     , '$(EHDB)'
                                                     , '$(EHA_SCHEMA)'
                                                     , '$(BACKUP_ACTIVITY_TABLE)'
                                                     , 'Colophon'
    ...
    IF (SELECT TOP(1) Colophon
      ...
      BEGIN
        IF @ForceNew <> 1
          RAISERROR($(MESSAGE_OFFSET)24, 16, 1, @DbName, @NodeName ); 
        ELSE
    ...
    IF @ReturnCode <> 0
      RAISERROR($(MESSAGE_OFFSET)22, 16, 1, @NodeName, '', '', '', @ActionType, @ReturnCode );
    ELSE
      RAISERROR($(MESSAGE_OFFSET)21, 0, 0, @NodeName, '', '', '', @ActionType );
    INSERT $(EHA_SCHEMA).$(BACKUP_ACTIVITY_TABLE) 
      ( Id
      , DbName
      , Node
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    SELECT TOP(1) @BackupName = k.BackupName
                ...
                , @RestoreDDL = 
                    FORMATMESSAGE ( $(MESSAGE_OFFSET)22  
                                  , @DbName   
...
CREATE PROCEDURE $(EHA_SCHEMA).BackupDatabaseMasterKey 
 ( @DbName NVARCHAR(128)
 , @BackupPhrase NVARCHAR(128)
 , @KeyPhrase NVARCHAR(128) = NULL  -- NULL if no encryption PHRASE used 
 , @UseHash TINYINT = 0             -- use BackupName (0) clear text or BackupNameBucket (1) as file name
 , @ForceNew TINYINT = 0 )          -- if 1 backup even if backup of this uniquely identified key already on file
$(WITH_OPTIONS)
AS
BEGIN 
 DECLARE @ActionType NVARCHAR(10) = 'Backup'
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)13
                                                 , @DbName
                                                 , @BackupPhrase
                                                 , @KeyPhrase
                                                 , @ForceNew)
    ...
    SET @BackupDDL = FORMATMESSAGE( $(MESSAGE_OFFSET)26
                                   ...
                                   , CASE WHEN DB_NAME() <> @DbName AND @KeyPhrase IS NOT NULL
                                          THEN FORMATMESSAGE ($(MESSAGE_OFFSET)43, @KeyPhrase )
                                          ELSE '' END
    ...
    EXEC @ReturnCode = sp_executesql @BackupDDL;
    IF @ReturnCode <> 0
      RAISERROR($(MESSAGE_OFFSET)22,16,1,'DATABASE', @DbName,'Master Key','', @ActionType, @ReturnCode);
    ELSE
      RAISERROR($(MESSAGE_OFFSET)21,0,0,'DATABASE', @DbName,'Master Key','', @ActionType);
    INSERT $(EHA_SCHEMA).$(BACKUP_ACTIVITY_TABLE) 
      ( Id
      , DbName
      , Node
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)14
                                                 , @DbName
                                                 , @IdToRestore
                                                 , @ForceReplace)
                                  , 1
    ...
    IF @ReturnCode <> 0 
      RAISERROR($(MESSAGE_OFFSET)22, 16, 1, 'DATABASE', @DbName, @NodeName, '', @ActionType, @ReturnCode );
    ELSE
      RAISERROR($(MESSAGE_OFFSET)21, 0, 0, 'DATABASE', @DbName, @NodeName, '', @ActionType );
...
CREATE PROCEDURE $(EHA_SCHEMA).BackupCertificate 
 ...
 , @DbName NVARCHAR(128) 
 , @BackupPhrase NVARCHAR(128) = NULL -- use stored value if exists 
 , @KeyPhrase NVARCHAR(128) = NULL    -- value needed only when type = PW
 , @UseHash TINYINT = 0               -- use BackupName (0) clear text or BackupNameBucket (1) as file name
 , @ForceNew TINYINT = 0 )            -- if 1 backup even if backup of this uniquely identified key already on file
$(WITH_OPTIONS)
AS
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)15
                                                 , @CertificateName
                                                 , @DbName
                                                 , @BackupPhrase
                                                 , @KeyPhrase
    ...
    SET @ObjectInfoDDL = FORMATMESSAGE( $(MESSAGE_OFFSET)46
                                      , FORMATMESSAGE( $(MESSAGE_OFFSET)44
                                                     , '$(EHDB)'
                                                     , '$(EHA_SCHEMA)'
                                                     , '$(BACKUP_ACTIVITY_TABLE)'
    ...
    IF (SELECT TOP(1) Colophon
      ...
      BEGIN
        IF @ForceNew <> 1
          RAISERROR($(MESSAGE_OFFSET)24, 16, 1, @DbName, @CertificateName ); 
        ELSE
          SET @Edition = (SELECT MAX(Edition) + 1
                          FROM  $(EHA_SCHEMA).$(BACKUP_ACTIVITY_TABLE) 
                          WHERE ServerName = @@SERVERNAME
    ...
    SET @BackupDDL = (SELECT FORMATMESSAGE ( $(MESSAGE_OFFSET)27
                                           ...
                                           , CASE WHEN @DMKPhraseName IS NOT NULL -- need to open master key
                                                   THEN (SELECT FORMATMESSAGE ( $(MESSAGE_OFFSET)43
                                                                              , CAST( DECRYPTBYKEY( Value
                                                                                                 , 1
                                                                                                 , CAST ( DECRYPTBYKEY( Name ) AS NVARCHAR(448) ) 
                                                                                                 ) AS NVARCHAR(128) ) )
    ...
    EXEC @ReturnCode = sp_executesql @BackupDDL;
    IF @ReturnCode <> 0
      RAISERROR($(MESSAGE_OFFSET)22,16,1,'DATABASE', @DbName, 'CERTIFICATE', @CertificateName,'BACKUP',@ReturnCode);
    ELSE
      RAISERROR($(MESSAGE_OFFSET)21, 0,0,'DATABASE', @DbName, 'CERTIFICATE', @CertificateName,'BACKUP');
    INSERT $(EHA_SCHEMA).$(BACKUP_ACTIVITY_TABLE) 
      ( Id
      , DbName
      , Node
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)16
                                                 , @CertificateName
                                                 , @DbName
                                                 , @IdToRestore)
                                  , 1
    ...
    IF (@CertificateId IS NOT NULL) -- certificate exist - cannot restore
      BEGIN
        ...
        RAISERROR($(MESSAGE_OFFSET)23,16,1,'DATABASE', @DbName, 'CERTIFICATE',@CertificateName,'RESTORE',@Reason);
      END
    ...
    SET @KeyPhraseName = $(EHA_SCHEMA).GetEHPhraseName(@DbName, @CertificateName, 'Encryption', @Id );
    INSERT @Keytvp (Name, Value)
...
SET NOCOUNT ON;
  BEGIN TRY
    SET @SignedReferences = $(EHA_SCHEMA).VerifySchema(@@PROCID);
    OPEN SYMMETRIC KEY [$(AUDIT_SYMMETRIC_KEY)]
    DECRYPTION BY CERTIFICATE [$(AUDIT_CERTIFICATE)]
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)07
                                                 , @FileName, @Tag )
                                  , 1
                                  , @Id );
    CLOSE SYMMETRIC KEY [$(AUDIT_SYMMETRIC_KEY)];
    ...
    IF @ReturnCode <> 0  
      RAISERROR($(MESSAGE_OFFSET)22,16,1,'@GetContainerDDL', @FilePath, @FileName, '', '' ,@ReturnCode);
    INSERT $(EHA_SCHEMA).$(CONTAINER_ACTIVITY_TABLE) 
      ( Id
      , FileName
      , FilePath
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)09
                                                 , @Id, @FileName )
                                  , 1
                                  , @Id );
    CLOSE SYMMETRIC KEY [$(AUDIT_SYMMETRIC_KEY)];
...
SET NOCOUNT ON;
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)03
...
DECLARE @MAC VARBINARY(128)
  ...
  BEGIN TRY
    ...
    WITH PASSWORD = '$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)';
    SET @Parameters = ENCRYPTBYKEY( Key_GUID('$(AUDIT_SYMMETRIC_KEY)')
                                  , FORMATMESSAGE( $(MESSAGE_OFFSET)08, @ServerName )
                                  , 1
                                  , @Id );
    CLOSE SYMMETRIC KEY [$(AUDIT_SYMMETRIC_KEY)];
    EXEC $(EHA_SCHEMA).Book @@PROCID
...
SELECT $(OBJECT_COUNT) AS [Expected Object Count]
     , $(TABLE_COUNT) AS [Expected Table Count]
     , COUNT(*) AS [Actual Object Count] 
     , SUM(ISNULL(is_signed,0)) AS [Total Signature Count]
...
GO
EXEC $(EHA_SCHEMA).BackupServiceMasterKey @BackupPhrase =N'$(SMK_BACKUP_PHRASE)'
                                        , @UseHash = $(USE_HASH_FOR_FILENAME)
                                        , @ForceNew = DEFAULT; 
GO
EXEC $(EHA_SCHEMA).BackupDatabaseMasterKey @DbName = N'master'
                                         , @BackupPhrase = N'$(master_DMK_BACKUP_PHRASE)'
                                         , @KeyPhrase = N'$(master_DMK_ENCRYPTION_PHRASE)'
                                         , @UseHash = $(USE_HASH_FOR_FILENAME)
                                         , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupDatabaseMasterKey @DbName = N'$(EHDB)'
                                         , @BackupPhrase = N'$(EHDB_DMK_BACKUP_PHRASE)'
                                         , @KeyPhrase = N'$(EHDB_DMK_ENCRYPTION_PHRASE)'
                                         , @UseHash = $(USE_HASH_FOR_FILENAME)
                                         , @ForceNew = DEFAULT;
GO
-- Master Key encrypted certs need a backup file encryption password 
IF EXISTS (SELECT * FROM master.sys.certificates 
    ...
    EXEC $(EHA_SCHEMA).BackupCertificate @CertificateName = N'$(TDE_CERTIFICATE)'
                                       , @DbName = N'master'
                                       , @BackupPhrase = N'$(TDE_CERTIFICATE_BACKUP_PHRASE)'
                                       , @KeyPhrase = DEFAULT
                                       , @UseHash = $(USE_HASH_FOR_FILENAME)
                                       , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupCertificate @CertificateName = N'$(NAME_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(NAME_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = DEFAULT
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupCertificate @CertificateName = N'$(VALUE_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(VALUE_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = DEFAULT
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupCertificate @CertificateName = N'$(FILE_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(FILE_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = DEFAULT
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupCertificate @Certificatename = N'$(AUTHENTICITY_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(AUTHENTICITY_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = DEFAULT
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
...
EXEC $(EHA_SCHEMA).BackupCertificate @CertificateName = N'$(AUDIT_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(AUDIT_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = N'$(AUDIT_CERTIFICATE_ENCRYPTION_PHRASE)'
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupCertificate @Certificatename = N'$(EVENT_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(EVENT_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = DEFAULT
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).BackupCertificate @CertificateName = N'$(OBJECT_CERTIFICATE)'
                                   , @DbName = N'$(EHDB)'
                                   , @BackupPhrase = N'$(OBJECT_CERTIFICATE_BACKUP_PHRASE)'
                                   , @KeyPhrase = N'$(OBJECT_CERTIFICATE_ENCRYPTION_PHRASE)'
                                   , @UseHash = $(USE_HASH_FOR_FILENAME)
                                   , @ForceNew = DEFAULT;
GO
EXEC $(EHA_SCHEMA).SavePortableSymmetricKey '$(ERROR_SYMMETRIC_KEY)'
                                          , '$(ERROR_KEY_IDENTITY)'
                                          , '$(ERROR_KEY_SOURCE)';
GO
EXEC $(EHA_SCHEMA).SavePrivateValue 'LINK_PASSWORD'
                                  , '$(LINK_PASSWORD)'
                                  , '$(PRIVATE_ENCRYPTION_PHRASE)'
GO
EXEC $(EHA_SCHEMA).SavePrivateValue 'TRUECRYPT_CONTAINER_ENCRYPTION_PHRASE'
                                  , '$(TRUECRYPT_CONTAINER_ENCRYPTION_PHRASE)'
                                  , '$(PRIVATE_ENCRYPTION_PHRASE)';
GO
